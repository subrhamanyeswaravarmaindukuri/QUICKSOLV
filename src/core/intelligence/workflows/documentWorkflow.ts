...
export class DocumentWorkflow implements QuickSolvWorkflow {
  readonly taskType: QuickSolvTaskType = "DOCUMENT";
  ...
  async execute(
    ...
    if (request.pdf) {
      effectivePrompt = `[Attached Document Context]: PDF (${request.pdf.mimeType})\n${request.prompt}`;
    }
